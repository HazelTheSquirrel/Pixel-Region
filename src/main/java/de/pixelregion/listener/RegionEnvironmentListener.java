...
public final class RegionEnvironmentListener implements Listener {
    ...
    public RegionEnvironmentListener(RegionPolicyService policy) { this.policy = policy; }
    ...
    public void onBlockFromTo(BlockFromToEvent event) {
        if (!policy.allows(event.getToBlock().getLocation(), RegionFlag.FLUID_FLOW, null)) event.setCancelled(true);
    }

    // Prevents fire ignition when the destination denies FIRE_SPREAD.
    @EventHandler(ignoreCancelled = true)
    public void onIgnite(BlockIgniteEvent event) {
        if (!policy.allows(event.getBlock().getLocation(), RegionFlag.FIRE_SPREAD, null)) event.setCancelled(true);
    }
}
